...
function TechStack() {
  ...
  { name: "Power BI", icon: <SiKotlin className="text-yellow-500" size={32} /> },
...
const iconSize = 80; // px, matches w-20 h-20
  const ringGap = 50; // px, gap between rings
  const baseRadius = 120; // px, inner ring radius
  ...
  return (
     <div className="pt-10 relative overflow-hidden">
      ...
      <div
        className="absolute top-32 right-80 w-[900px] h-[750px]
                   bg-gradient-to-r from-purple-700 via-indigo-700 to-sky-400
                   rounded-full blur-3xl opacity-30 pointer-events-none"
      />
        ...
        className="relative w-[700px] h-[700px] mx-auto">
          ...
          {rings.map((ring, ringIdx) => {
            return Array.from({ length: ring.count }).map((_, i) => {
              ...
              const x = 350 + ring.radius * Math.cos(angle) - iconSize / 2;
              const y = 350 + ring.radius * Math.sin(angle) - iconSize / 2;
              const tech = techStack[iconIndex++];
              return (
                <div
                  key={`${ringIdx}-${i}`}
                  className="flex flex-col items-center justify-center w-20 h-20 rounded-full bg-black/90 shadow-xl hover:scale-110 hover:bg-cyan-900/80 transition-transform cursor-pointer absolute border-2 border-cyan-400/20"
                  style={{
                    left: `${x}px`,
                    top: `${y}px`,
                    zIndex: 5,
